...
**Closure:** a PETG peg buckle cast into the end of the short strap. A plate closes the strap end (its outer face shows), two tongues with holes anchor it mid-thickness in the strap, two rails carry a crossbar with a peg pointing at the wrist: a 2.4 mm neck long enough to pass through the strap and 1.2 mm beyond, with a one-sided 0.9 mm bump that the 3.0 mm hole snaps over, so it stays put when the band is slack. The long strap passes under the crossbar, the peg snaps into one of seven cast holes (5 mm pitch, sizes 155-185 mm), and the tail tucks under the short strap, whose last 12 mm are lifted 3 mm to let it through. Rigid frame, rigid peg: a one-thumb press, unlike v1's silicone stud.

| File | What | Print |
|---|---|---|
| `stl/band_buckle.stl` | peg buckle | **PETG**, as exported (frame flat, peg up), brim, 100% infill, 0.16 mm, **supports on, build plate only**: only the two tongues get one, a 0.8 mm pad each; the marks are on faces that end up buried in the strap |
