// Sleeve v4: v3's sealed jacket with the PETG frame moved OUTSIDE the silicone, flush
// with the back, so the Stick sits 1.5 mm off the wrist (0.5 mm silicone skin + 1.0 mm
// frame) instead of 2.4, and the lip over the face is 0.8. 16.1 mm tall, 17.0 at the
// front button. The frame's back faces are bare PETG, like a watch caseback: two end
// brackets carrying the fins and two rails along the sides. Fins grow straight off the
// brackets, so no PETG passes through silicone anywhere.
//
// Parts (set `part`, or use the Customizer):
//   "chassis"  PETG x1, 100% infill. Print as exported (skin-side face on the bed, fins
//              up), brim, no supports.
//   "core"     PLA. Print as exported (back face down), no supports.
//   "cup"      PLA. Open-top mold, straight rim with a rebate for the lid ridge, two M3
//              clamp holes and two M4 jack holes in the floor. Print as exported.
//   "lid"      PLA, required. Its underside forms the sleeve's back: a fillet ridge rounds
//              the edge, and four slightly tight slots grip the fins so the frame hangs
//              from the lid with its back faces pressed against it (bare, no film).
//   "sleeve"   preview of the silicone (don't print).
//
// Coordinates: x along the Stick (USB-C end at -x), y across, z from the back (0, wrist
// side) toward the front. The cup's rim is the z = 0 plane.
//
// Casting (open pour, window face down):
//   1. Release on cup, core, chassis, lid. Core on the floor, pad down, two M3x8 screws up
//      through the cup underside into it. Push the lid onto the chassis fins (dry) until
//      the frame's back is flat against the lid's underside.
//   2. Mix 12 g A + 12 g B (+ pigment). Syringe the lip layer and the front-button pocket
//      first, then pour to about 1 mm above the rim.
//   3. Lower lid + chassis on together, one end first so air runs out from under the
//      brackets, and press the lid onto the rim. Excess comes out of the vents and slots.
//   4. Cure 24 h (or ~6 h at 45 C).
//   5. Cut the flash round the lid's edge, lever a corner: the lid lifts off and the frame
//      stays in the sleeve. M3 screws out. Card down the long sides. Two M4 screws into the
//      jack holes, turned alternately, push the core and sleeve out. Peel the short-lip end
//      off the core, slide the core out through the window. Cut the port slit.
...
lip_t = 0.8;            // over the Stick's face: retains and seals, sets the height
skin_over = 0.5;        // silicone between the Stick's back and the frame (the sealed pocket floor)
side_bump = 0.5;        // extra over the side buttons
front_min = 0.8;        // silicone over the front button's top
front_chamfer = 1.0;
back_r = 0.8;           // round on the back (wrist-side) edge, formed by a fillet ridge on the LID (never an undercut in the cup)

/* [Chassis] */
frame_t = 1.0;          // frame thickness (brackets and rails), flush with the back surface
rail_w = 2.2;           // side rails joining the brackets (tension members)
bracket_l = 10.0;       // bracket length along the Stick (covers the fins)
slot_pinch = 0.1;       // lid slots are this much narrower than the fins (light press fit)
...
// ---- Chassis ---------------------------------------------------------------------------
CH_Z0 = 0; CH_Z1 = frame_t;                            // frame from the back surface up to the skin
...
echo(str("Sleeve v4 ", 2 * END_X, " x ", 2 * BODY_HW, " x ", TOP, " mm (+", FRONT_BUMP, " button bump); back ", back_t, "; fins ", fin_drop,
...
module fin_profile() {   // x-z, +x end
    hull() {
        translate([FIN_X0, 0]) square([fin_l, frame_t - 0.1]);   // overlaps into the bracket
...
module frame_raw() {   // undersize by edge_r, extended below the back plane (cut off later)
    for (m = [0, 1]) mirror([m, 0, 0])
        translate([CH_HL - bracket_l + edge_r, -(CH_HW - edge_r), -1]) cube([bracket_l - 2 * edge_r, 2 * (CH_HW - edge_r), 1 + frame_t - edge_r]);
    for (s = [-1, 1])
        translate([-(CH_HL - bracket_l + 0.5), s * (LUG_Y0 + rail_w / 2) - (rail_w / 2 - edge_r), -1]) cube([2 * (CH_HL - bracket_l + 0.5), rail_w - 2 * edge_r, 1 + frame_t - edge_r]);
}
module chassis_body() {
    // frame: rounded on the silicone side, flat and square-edged on the exposed back face
    intersection() {
        minkowski() { frame_raw(); sphere(r = edge_r, $fn = 16); }
        translate([-50, -50, 0]) cube([100, 100, 50]);
    }
    // fins, fully rounded, growing off the brackets' back faces
...
module lid() {
    difference() {
        union() {
            translate([0, 0, -LID_T]) linear_extrude(LID_T) rrect(2 * MX, 2 * MY, 4);
            lid_ridge();
        }
        // press-fit slots for the four fins
        for (m = [0, 1]) mirror([m, 0, 0]) for (s = [-1, 1])
            translate([FIN_X0 - 0.15, s * (LUG_Y0 + fin_t / 2) - (fin_t - slot_pinch) / 2, -LID_T - 1]) cube([fin_l + 0.3, fin_t - slot_pinch, LID_T + 2]);
        // vents over the middle of the back
        for (x = [-8, 8]) translate([x, 0, -LID_T - 1]) cylinder(d = 2.5, h = LID_T + 2);
    }
}

// ---- Exports ---------------------------------------------------------------------------------------------
if (part == "chassis")      translate([0, 0, CH_Z1]) rotate([180, 0, 0]) chassis();     // skin-side face on the bed, fins up
else if (part == "core")    translate([0, 0, -Z0]) core_part();
else if (part == "cup")     translate([0, 0, CUP_H]) rotate([180, 0, 0]) cup();          // open top up
else if (part == "lid")     translate([0, 0, LID_T]) lid();                              // flat, ridge up
...
else if (part == "check_lid")     intersection() { lid(); cup(); }                                 // must be empty
else if (part == "check_lidch")   intersection() { lid(); chassis(); }                             // only the slot pinch
else silicone();
